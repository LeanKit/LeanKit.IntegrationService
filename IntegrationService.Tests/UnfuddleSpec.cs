...
namespace IntegrationService.Tests.UnFuddle
{
	...
	public class When_updating_properties_of_target_item : UnfuddleSpec
	{
		...
		protected override void OnArrange()
		{
			...
			var restResponse1 = new RestResponse()
			{
				Content = serializer.SerializeToString(ticket1),
				StatusCode = HttpStatusCode.OK
			};

			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/1") && y.Method == Method.GET)))
				.Returns(restResponse1);
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/1") && y.Method == Method.PUT)))
				.Returns(restResponse1);
			...
			var restResponse2 = new RestResponse()
			{
				Content = serializer.SerializeToString(ticket2),
				StatusCode = HttpStatusCode.OK
			};
			...
			var restResponse3 = new RestResponse()
			{
				Content = serializer.SerializeToString(ticket3),
				StatusCode = HttpStatusCode.OK
			};

			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/3") && y.Method == Method.GET)))
				.Returns(restResponse3);
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/3") && y.Method == Method.PUT)))
				.Returns(restResponse3);
			...
			{
				Content = serializer.SerializeToString(ticket4),
				StatusCode = HttpStatusCode.OK
			};
		...
		protected override void OnStartTest()
		{
			TestItem = new TestUnfuddle(SubscriptionManager, ConfigurationProvider, LocalStorage, LeanKitClientFactory,
				RestClient);
		}
		...
		public void It_should_not_call_unfuddle_to_update_ticket_if_properties_do_not_change()
		{
			...
			MockRestClient.Verify(
				...
				Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/2") && y.Method == Method.PUT)), Times.Never());
		}
		...
		public void It_should_not_call_unfuddle_to_update_ticket_if_no_identified_properties_change()
		{
			...
			MockRestClient.Verify(
				...
				Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/4") && y.Method == Method.PUT)), Times.Never());
		}
		...
		public void It_should_not_call_unfuddle_to_update_ticket_externalsystemname_does_not_match()
		{
			...
			((TestUnfuddle) TestItem).TestCardUpdated(card, new List<string>(), _mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/5") && y.Method == Method.GET)), Times.Never());
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/5") && y.Method == Method.PUT)), Times.Never());
		}
	}
	...
	public class When_updating_state_of_target_item : UnfuddleSpec
	{
		...
		protected override void OnArrange()
		{
			...
			{
				Content = errorSerializer.SerializeToString(new Unfuddle.ErrorMessage() {Message = "Error"}),
		...
		public void It_should_not_call_unfuddle_to_update_ticket_if_card_does_not_have_external_id()
		{
			...
			((TestUnfuddle) TestItem).TestUpdateStateOfExternalItem(card, _mapping.LaneToStatesMap[2], _mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/2") && y.Method == Method.GET)), Times.Never());
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/2") && y.Method == Method.PUT)), Times.Never());
		}
		...
		public void It_should_not_call_unfuddle_to_update_ticket_if_unfuddle_does_not_have_matching_issue()
		{
			...
			MockRestClient.Verify(
				...
				Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/3") && y.Method == Method.PUT)), Times.Never());
		}
		...
		public void It_should_not_call_unfuddle_to_update_ticket_if_externalsystemname_does_not_match()
		{
			...
			((TestUnfuddle) TestItem).TestUpdateStateOfExternalItem(card, _mapping.LaneToStatesMap[2], _mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/4") && y.Method == Method.GET)), Times.Never());
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("tickets/4") && y.Method == Method.PUT)), Times.Never());
		}
	}
	...
	public class When_syncronizing_with_target_system : UnfuddleSpec
	{
		...
		protected override void OnArrange()
		{
			...
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("projects/1/ticket_reports") && y.Method == Method.GET)))
				.Returns(restResponse1);
			...
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("projects/2/ticket_reports") && y.Method == Method.GET)))
				.Returns(restResponse1);
			...
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("projects/3/ticket_reports") && y.Method == Method.GET)))
				.Returns(restResponse3);
			...
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("projects/4/ticket_reports") && y.Method == Method.GET)))
				.Returns(restResponse1);
			MockLeanKitApi.Setup(x => x.GetCardByExternalId(4, It.IsAny<string>()))
				.Returns(new Card() {Id = 4, ExternalSystemName = "Unfuddle"});
			MockLeanKitApi.Setup(x => x.AddCard(4, It.IsAny<Card>(), It.IsAny<string>())).Returns(_testCardAddResult1);

			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("projects/5/ticket_reports") && y.Method == Method.GET)))
				.Returns(restResponse1);
			MockLeanKitApi.Setup(x => x.GetCardByExternalId(5, It.IsAny<string>()))
				.Returns(new Card() {Id = 4, ExternalSystemName = "Unfuddlest"});
