...
namespace IntegrationService.Tests.JIRA
{
	public class JiraSpec : IntegrationBaseSpec
	{
		...
		protected Mock<IJiraConnection> MockJiraConnection;
		protected IJiraConnection JiraConnection;
		...
		protected override void OnCreateMockObjects()
		{
			...
			MockJiraConnection = new Mock<IJiraConnection>();
			JiraConnection = MockJiraConnection.Object;
		}
		...
		protected override void OnArrange()
		{
			...
			MockJiraConnection.Setup(x => x.GetSessionCookie(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<string>()))
	...
	[TestFixture]
	public class When_normalizing_descriptions
	{
		[Test]
		public void It_should_convert_JIRA_plain_text_to_LeanKit_HTML()
		{
			var given = "This is going to be \"epic\" y'all!\r\n\r\nJIRA AGREES 2.";
			var expected = "<p>This is going to be \"epic\" y'all!</p>\n<p>JIRA AGREES 2.</p>";

			var response = given.JiraPlainTextToLeanKitHtml();
			response.ShouldEqual(expected);
		}

		[Test]
		public void It_should_convert_LeanKit_html_to_Jira_plain_text()
		{
			var given = "<p>This is going to be \"epic\" y'all!</p>\\n<p>JIRA AGREES 2.</p>";
			var expected = "This is going to be \\\"epic\\\" y'all!\\r\\n\\r\\nJIRA AGREES 2.";

			var response = given.LeanKitHtmlToJiraPlainText();
			response.ShouldEqual(expected);
		}

		[Test]
		public void It_should_convert_LeanKit_html_with_line_breaks_to_Jira_plain_text()
		{
			var given = "<p>This is going to be \"epic\" y'all!</p>\\n<p>JIRA AGREES 2.<br />Just a line break.</p>";
			var expected = "This is going to be \\\"epic\\\" y'all!\\r\\n\\r\\nJIRA AGREES 2.\\r\\nJust a line break.";
	...
	public class When_calculating_card_type : JiraSpec
	{
		...
		{
			JiraConversionExtensions.CalculateLeanKitCardType(_mapping, "Willy")
		...
		{
			JiraConversionExtensions.CalculateLeanKitCardType(_mapping, "Roger")
				.Id.ShouldEqual(_testBoard.CardTypes.FirstOrDefault(x => x.Name == "Willy").Id);
		}
	}
	...
	public class When_creating_a_target_item : JiraSpec
	{
		...
		private class TestJira : Jira
		{
			public TestJira(IBoardSubscriptionManager subscriptions,
				...
				: base(subscriptions, configurationProvider, localStorage, leanKitClientFactory, restClient, jiraConnection)
			{
			}
	...
	public class When_updating_properties_of_target_item : JiraSpec
	{
		...
		public void It_should_not_call_jira_to_update_issue_if_externalsystemname_does_not_match()
		{
			...
			MockRestClient.Verify(
				...
				Times.Never());
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/latest/issue/five") && y.Method == Method.PUT)),
				Times.Never());
		}
	}
	...
	public class When_updating_state_of_target_item : JiraSpec
	{
		...
		public void It_should_not_call_jira_to_update_ticket_if_externalsystemname_does_not_match()
		{
			...
			((TestJira) TestItem).TestUpdateStateOfExternalItem(card, _mapping.LaneToStatesMap[2], _mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("latest/issue/siz") && y.Method == Method.GET)),
				Times.Never());
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("2/issue/six/transitions") && y.Method == Method.GET)),
				Times.Never());
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issue/six/transitions") && y.Method == Method.POST)),
				Times.Never());
		}
	}
	...
	public class When_updating_state_of_target_item_through_workflow : JiraSpec
	{
		...
		protected override void OnArrange()
		{
			...
			var transitions1 = new Jira.TransitionsResponse()
			{
				Transitions = new List<Jira.Transition>()
				{
					new Jira.Transition()
					{
						Id = "3",
						Name = "Active",
						To = new Jira.Status() {Name = "Active", Description = "Active", Id = "3"}
					},
					new Jira.Transition()
					{
						Id = "4",
						Name = "Resolved",
						To = new Jira.Status() {Name = "Resolved", Description = "Resolved", Id = "4"}
					},
					new Jira.Transition()
					{
						Id = "2",
			...
			var restTransitionsResponse3 = new RestResponse()
			{
				Content = transitionsSerializer.SerializeToString(transitions3),
				StatusCode = HttpStatusCode.OK
			};
			...
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("2/issue/three/transitions") && y.Method == Method.GET)))
				.Returns(restTransitionsResponse3);
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issue/three/transitions") && y.Method == Method.POST)))
				.Returns(restResponse3);
			...
			var restTransitionsResponse4 = new RestResponse()
			{
				Content = transitionsSerializer.SerializeToString(transitions4),
				StatusCode = HttpStatusCode.OK
			};
			...
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("2/issue/four/transitions") && y.Method == Method.GET)))
				.Returns(restTransitionsResponse4);
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issue/four/transitions") && y.Method == Method.POST)))
				.Returns(restResponse4);
		}
