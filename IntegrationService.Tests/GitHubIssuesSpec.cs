...
namespace IntegrationService.Tests.GitHub.Issues
{
	public class GitHubIssuesSpec : IntegrationBaseSpec
	{
		...
		{
			TestItem = new GitHubIssues(SubscriptionManager, ConfigurationProvider, LocalStorage, LeanKitClientFactory,
				RestClient);
		}
	}
	...
	public class When_calculating_card_type : GitHubIssuesSpec
	{
		...
		public void It_should_return_default_card_type_if_issue_has_no_labels()
		{
			...
			GithubConversionExtensions.CalculateLeanKitCardType(_mapping, (GitHubIssues.Issue) null)
		...
		public void It_should_return_default_card_type_if_issue_has_no_matching_labels()
		{
			var issue = new GitHubIssues.Issue()
			{
				Labels = new List<GitHubIssues.Label>() {new GitHubIssues.Label() {Name = "Bob"}}
			};
			GithubConversionExtensions.CalculateLeanKitCardType(_mapping, issue).Id.ShouldEqual(_testBoard.CardTypes.Last().Id);
		}
		...
		public void It_should_return_implicit_card_type_if_issue_has_matching_label()
		{
			var issue = new GitHubIssues.Issue()
			{
				Labels = new List<GitHubIssues.Label>() {new GitHubIssues.Label() {Name = "Willy"}}
			};
			GithubConversionExtensions.CalculateLeanKitCardType(_mapping, issue)
		...
		public void It_should_return_mapped_card_type_if_issue_has_matching_label()
		{
			var issue = new GitHubIssues.Issue()
			{
				Labels = new List<GitHubIssues.Label>() {new GitHubIssues.Label() {Name = "Roger"}}
			};
			GithubConversionExtensions.CalculateLeanKitCardType(_mapping, issue)
				.Id.ShouldEqual(_testBoard.CardTypes.FirstOrDefault(x => x.Name == "Willy").Id);
		}
	...
	public class When_updating_properties_of_target_item : GitHubIssuesSpec
	{
		...
		protected override void OnArrange()
		{
			...
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/4") && y.Method == Method.GET)))
				.Returns(restResponse4);
			MockRestClient.Setup(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/4") && y.Method == Method.PATCH)))
				.Returns(restResponse4);
		}

		protected override void OnStartTest()
		{
			TestItem = new TestGitHubIssues(SubscriptionManager, ConfigurationProvider, LocalStorage, LeanKitClientFactory,
				RestClient);
		}
		...
		public void It_should_call_github_to_update_issue_if_many_properties_change()
		{
			...
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/1") && y.Method == Method.GET)), Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/1") && y.Method == Method.PATCH)),
				Times.Exactly(1));
		}
		...
		public void It_should_not_call_github_to_update_issue_if_properties_do_not_change()
		{
			...
			((TestGitHubIssues) TestItem).TestCardUpdated(card, new List<string>() {"Title", "Description"}, _mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/2") && y.Method == Method.GET)), Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/2") && y.Method == Method.PATCH)), Times.Never());
		}
		...
		public void It_should_call_github_to_update_issue_if_one_property_changes()
		{
			...
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/3") && y.Method == Method.GET)), Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/3") && y.Method == Method.PATCH)),
				Times.Exactly(1));
		}
		...
		public void It_should_not_call_github_to_update_issue_if_no_identified_properties_change()
		{
			...
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/4") && y.Method == Method.GET)), Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/4") && y.Method == Method.PATCH)), Times.Never());
		}
		...
		public void It_should_not_call_github_to_update_issue_if_externalsystemname_does_not_match()
		{
			...
			((TestGitHubIssues) TestItem).TestCardUpdated(card, new List<string>(), _mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/5") && y.Method == Method.GET)), Times.Never());
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/5") && y.Method == Method.PATCH)), Times.Never());
		}
	}
	...
	public class When_updating_state_of_target_item : GitHubIssuesSpec
	{
		...
		protected override void OnArrange()
		{
			...
			{
				Content = errorSerializer.SerializeToString(new GitHubIssues.ErrorMessage() {Message = "Error"}),
		...
		public void It_should_not_call_github_to_update_issue_if_card_does_not_have_external_id()
		{
			...
			((TestGitHubIssues) TestItem).TestUpdateStateOfExternalItem(card, _mapping.LaneToStatesMap[2], _mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/2") && y.Method == Method.GET)), Times.Never());
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/2") && y.Method == Method.PATCH)), Times.Never());
		}
		...
		public void It_should_not_call_github_to_update_issue_if_github_does_not_have_matching_issue()
		{
			...
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/3") && y.Method == Method.GET)), Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/3") && y.Method == Method.PATCH)), Times.Never());
		}
		...
		public void It_should_not_call_github_to_update_issue_if_externalsytemname_does_not_match()
		{
			...
			((TestGitHubIssues) TestItem).TestUpdateStateOfExternalItem(card, _mapping.LaneToStatesMap[2], _mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/4") && y.Method == Method.GET)), Times.Never());
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("issues/4") && y.Method == Method.PATCH)), Times.Never());
		}
	}
	...
	public class When_syncronizing_with_target_system : GitHubIssuesSpec
	{
		...
		protected override void OnArrange()
		{
			...
			MockRestClient.Setup(
				x =>
					x.Execute(
						It.Is<RestRequest>(
							y => y.Resource.Contains("/1/issues") && y.Method == Method.GET && y.Parameters[0].Value.ToString() == "Open")))
				.Returns(restResponse1);
			MockRestClient.Setup(
				x =>
					x.Execute(
			...
			MockRestClient.Setup(
				x =>
					x.Execute(
						It.Is<RestRequest>(
							y => y.Resource.Contains("/2/issues") && y.Method == Method.GET && y.Parameters[0].Value.ToString() == "Open")))
				.Returns(restResponse1);
			MockRestClient.Setup(
				x =>
					x.Execute(
			...
			MockRestClient.Setup(
				x =>
					x.Execute(
						It.Is<RestRequest>(
							y => y.Resource.Contains("/3/issues") && y.Method == Method.GET && y.Parameters[0].Value.ToString() == "Open")))
				.Returns(restResponse3);
			MockRestClient.Setup(
				x =>
					x.Execute(
			...
			MockRestClient.Setup(
				x =>
					x.Execute(
						It.Is<RestRequest>(
							y => y.Resource.Contains("/4/issues") && y.Method == Method.GET && y.Parameters[0].Value.ToString() == "Open")))
				.Returns(restResponse1);
			MockRestClient.Setup(
				x =>
					x.Execute(
			...
			MockRestClient.Setup(
				x =>
					x.Execute(
						It.Is<RestRequest>(
							y => y.Resource.Contains("/5/issues") && y.Method == Method.GET && y.Parameters[0].Value.ToString() == "Open")))
				.Returns(restResponse1);
			MockRestClient.Setup(
				x =>
					x.Execute(
			...
			MockLeanKitApi.Setup(x => x.GetCardByExternalId(5, It.IsAny<string>()))
				.Returns(new Card() {Id = 4, ExternalSystemName = "GitHubby"});
		...
		public void It_should_call_github_to_get_list_of_issues()
		{
			...
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/1/issues") && y.Method == Method.GET)),
				Times.Exactly(_mapping.QueryStates.Count));
		...
		public void It_should_call_leankit_once_to_create_card_if_there_is_one_issue()
		{
			...
			((TestGitHubIssues) TestItem).Syncronize(_mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/2/issues") && y.Method == Method.GET)),
				Times.Exactly(_mapping.QueryStates.Count));
		...
		public void It_should_call_leankit_multiple_times_to_create_card_if_there_are_multiple_issues()
		{
			...
			((TestGitHubIssues) TestItem).Syncronize(_mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/3/issues") && y.Method == Method.GET)),
				Times.Exactly(_mapping.QueryStates.Count));
		...
		public void It_should_not_call_leankit_to_create_card_if_card_with_externalid_already_exists()
		{
			...
			((TestGitHubIssues) TestItem).Syncronize(_mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/4/issues") && y.Method == Method.GET)),
				Times.Exactly(_mapping.QueryStates.Count));
		...
		public void It_should_call_leankit_to_create_card_if_card_with_externalid_exists_but_different_externalsystemname()
		{
			...
			((TestGitHubIssues) TestItem).Syncronize(_mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/5/issues") && y.Method == Method.GET)),
				Times.Exactly(_mapping.QueryStates.Count));
