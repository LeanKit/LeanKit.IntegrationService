...
namespace IntegrationService.Tests.GitHub.Pulls
{
	...
	public class When_updating_properties_of_target_item : GitHubPullsSpec
	{
		...
		protected override void OnStartTest()
		{
			TestItem = new TestGitHubPulls(SubscriptionManager, ConfigurationProvider, LocalStorage, LeanKitClientFactory,
				RestClient);
		}
		...
		public void It_should_call_github_to_update_pull_if_many_properties_change()
		{
			...
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("pulls/1") && y.Method == Method.GET)), Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("pulls/1") && y.Method == Method.PATCH)),
				Times.Exactly(1));
		}
		...
		public void It_should_call_github_to_update_pull_if_one_property_changes()
		{
			...
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("pulls/3") && y.Method == Method.GET)), Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("pulls/3") && y.Method == Method.PATCH)),
				Times.Exactly(1));
		}
	...
	public class When_updating_state_of_target_item : GitHubPullsSpec
	{
		...
		public void It_should_not_call_github_to_update_pull_if_card_does_not_have_external_id()
		{
			...
			((TestGitHubPulls) TestItem).TestUpdateStateOfExternalItem(card, _mapping.LaneToStatesMap[2], _mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("pulls/2") && y.Method == Method.GET)), Times.Never());
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("pulls/2") && y.Method == Method.PATCH)), Times.Never());
		}
		...
		public void It_should_not_call_github_to_update_pull_if_github_does_not_have_matching_issue()
		{
			...
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("pulls/3") && y.Method == Method.GET)), Times.Exactly(1));
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("pulls/3") && y.Method == Method.PATCH)), Times.Never());
		}
	...
	public class When_syncronizing_with_target_system : GitHubPullsSpec
	{
		...
		public void It_should_call_github_to_get_list_of_pullrequests()
		{
			...
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/1/pulls") && y.Method == Method.GET)), Times.Exactly(1));
		...
		public void It_should_call_leankit_once_to_create_card_if_there_is_one_pullrequest()
		{
			...
			((TestGitHubPulls) TestItem).Syncronize(_mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/2/pulls") && y.Method == Method.GET)), Times.Exactly(1));
		...
		public void It_should_call_leankit_multiple_times_to_create_card_if_there_are_multiple_pullrequests()
		{
			...
			((TestGitHubPulls) TestItem).Syncronize(_mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/3/pulls") && y.Method == Method.GET)), Times.Exactly(1));
		...
		public void It_should_not_call_leankit_to_create_card_if_card_with_externalid_already_exists()
		{
			...
			((TestGitHubPulls) TestItem).Syncronize(_mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/4/pulls") && y.Method == Method.GET)), Times.Exactly(1));
			MockLeanKitApi.Verify(x => x.AddCard(4, It.IsAny<Card>(), It.IsAny<string>()), Times.Never());
		}

		[Test]
		public void
			It_should_call_leankit_to_create_card_if_existing_card_has_externalid_but_externalsystemname_does_not_match()
		{
			_mapping.Identity.LeanKit = 5;
			_mapping.Identity.Target = "5";
			((TestGitHubPulls) TestItem).Syncronize(_mapping);
			MockRestClient.Verify(
				x => x.Execute(It.Is<RestRequest>(y => y.Resource.Contains("/5/pulls") && y.Method == Method.GET)), Times.Exactly(1));
